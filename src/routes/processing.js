...
router.get(
  '/',
  asyncHandler(processingController.listJobs)
);

// Get job statistics (viewer permission)
router.get(
  '/stats',
  asyncHandler(processingController.getStats)
);

// Get pending jobs (viewer permission)
router.get(
  '/pending',
  asyncHandler(processingController.getPendingJobs)
);

// Get failed jobs (viewer permission)
router.get(
  '/failed',
  asyncHandler(processingController.getFailedJobs)
);

// Get retryable jobs (viewer permission)
router.get(
  '/retryable',
  asyncHandler(processingController.getRetryableJobs)
);

// Get single job (viewer permission)
router.get(
  '/:id',
  asyncHandler(processingController.getJob)
);

// Get jobs for specific episode (viewer permission)
router.get(
  '/episode/:episodeId',
  asyncHandler(processingController.getEpisodeJobs)
);
