...
router.post('/:episodeId/script-suggestions', optionalAuth, async (req, res) => {
  try {
    ...
    // Try to get episode and template, but don't fail if they don't exist
    let episode = null;
    let template = null;

    try {
      episode = await db.Episode.findByPk(episodeId);
    } catch (e) {
      console.warn('Could not fetch episode:', e.message);
    }

    try {
      template = await db.ScriptTemplate.findByPk(template_id);
    } catch (e) {
      console.warn('Could not fetch template:', e.message);
    }

    // If we can't find either, just return empty suggestions
    if (!episode || !template) {
      return res.json({ success: true, data: [] });
    }
