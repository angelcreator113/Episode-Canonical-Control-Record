...
/**
 * The Social Personality Engine — Doc 06 · v1.0 · March 2026
 * franchise_law · always_inject
 *
 * 11 entries:
 *   1.  Core Personality Traits
 *   2.  Posting Archetypes
 *   3.  Social Motivation Types
 *   4.  Relationship Dynamics
 *   5.  Emotional Reaction System
 *   6.  Reputation System
 *   7.  Influence Growth Model
 *   8.  Social Reputation Damage & Recovery
 *   9.  Character Story Arcs
 *   10. Personality × Algorithm Interaction
 *   11. JustAWoman's Profile Note
 */
...
module.exports = {
  async up(queryInterface) {
    ...
    await queryInterface.bulkInsert('franchise_knowledge', [
      /* ─── 01 · Core Personality Traits ─── */
      {
        title: 'Core Personality Traits (0–100 Scale)',
        content: JSON.stringify({
          summary: 'Each character has a score in six traits. The score shapes behavior — not as rules but as tendencies that make characters feel consistent. High scores are not better than low scores. The most interesting characters are the ones whose trait scores are in conflict with each other.',
          traits: [
      ...
      /* ─── 02 · Posting Archetypes ─── */
      {
        title: 'Posting Archetypes',
        content: JSON.stringify({
          summary: 'Characters naturally fall into posting styles based on their trait scores. Most characters are a primary archetype with secondary tendencies. The archetype can shift across a character\'s arc — this is one of the signals that something has changed.',
          archetypes: [
        ...
        severity: 'critical',
        always_inject: true,
        applies_to: JSON.stringify(['story_generation', 'character_profiles', 'posting_behavior']),
        source_document: 'social-personality-v1.0',
        source_version: '1.0',
        created_at: now,
        updated_at: now
      ...
      /* ─── 03 · Social Motivation Types ─── */
      {
        title: 'Social Motivation Types',
        content: JSON.stringify({
          summary: 'These define why characters use social media. Motivation is not always what it looks like — a Fame Seeker can perform as a Community Builder. The gap between real motivation and performed motivation is story.',
          motivations: [
            ...
            { type: 'Community Builder', goal: 'Belonging, connection, being at the center of a real network', pattern: 'Group conversations, support culture, elevating other creators — the community is the product', gap: 'The audience sees generosity. The Community Builder needs the network as much as the network needs her.', breaks: "She builds something real and it gets taken from her. Or she realizes the community she built was for someone else's benefit." },
        ...
        severity: 'critical',
        always_inject: true,
        applies_to: JSON.stringify(['story_generation', 'character_profiles', 'social_decisions']),
        source_document: 'social-personality-v1.0',
        source_version: '1.0',
        created_at: now,
        updated_at: now
      ...
      /* ─── 04 · Relationship Dynamics ─── */
      {
        ...
        content: JSON.stringify({
          summary: "Characters interact through social bonds that shape their content, their audience, and their arcs. The relationship type determines what each person gets, what they risk, and what breaks when it ends.",
          types: [
        ...
        severity: 'critical',
        always_inject: true,
        applies_to: JSON.stringify(['story_generation', 'character_interactions', 'relationship_arcs']),
        source_document: 'social-personality-v1.0',
        source_version: '1.0',
        created_at: now,
        updated_at: now
      ...
      /* ─── 05 · Emotional Reaction System ─── */
      {
        title: 'Emotional Reaction System',
        content: JSON.stringify({
          summary: "Characters react differently to the same event based on their trait profile. The reaction type a character defaults to is one of the most revealing things about them — and the most consistent.",
          reactions: [
            ...
            { type: 'Withdrawn', triggers: "Overwhelm, significant personal event, content that didn't land as expected", feedLook: 'Going quiet — the Ghost mode activated by circumstance rather than personality', traits: 'Variable trait profile — this is a response to pressure, not a stable state', shift: "The silence after something happened. The return post carries the weight of everything she didn't say." }
          ]
        }),
        category: 'franchise_law',
        severity: 'critical',
        always_inject: true,
        applies_to: JSON.stringify(['story_generation', 'character_reactions', 'feed_scenes']),
        source_document: 'social-personality-v1.0',
        source_version: '1.0',
        created_at: now,
        updated_at: now
      ...
      /* ─── 06 · Reputation System ─── */
      {
        ...
        content: JSON.stringify({
          summary: "Each character develops a public reputation that travels ahead of them — the audience's expectation before they've seen the latest content. Reputation affects what opportunities arrive and what accusations stick.",
          types: [
            ...
            { type: 'Mysterious', built: 'Selective disclosure, strategic absence, content that implies more than it says', opens: 'Intense audience investment, the parasocial relationship that fills in the gaps, aspirational projection', attracts: "The audience that invents her. The image becomes larger than the person.", destroyed: "Revealing too much. Or having something revealed without her control. The mystery ends." }
          ]
        }),
        category: 'franchise_law',
        severity: 'critical',
        always_inject: true,
        applies_to: JSON.stringify(['story_generation', 'character_profiles', 'reputation_arcs']),
        source_document: 'social-personality-v1.0',
        source_version: '1.0',
        created_at: now,
        updated_at: now
      ...
      /* ─── 07 · Influence Growth Model ─── */
      {
        title: 'Influence Growth Model',
        content: JSON.stringify({
          summary: 'Creators gain influence through four forces. The fastest growth combines all four. The most sustainable growth is built on the first two.',
          forces: [
            { force: 'Consistency', requires: 'Showing up at the same frequency, quality level, and voice — even when nobody is watching', builds: 'Algorithmic momentum, audience habit, the sense that she is reliable', cannotBuild: 'Audience passion — consistency is respect, not devotion', risk: "The creator who has been consistent so long the audience stopped noticing. Invisible by reliability." },
            { force: 'Cultural Participation', requires: 'Engaging with events, trends, and cultural moments as they happen — Velvet Season content during Velvet Season', builds: 'Relevance, discoverability, the sense that she is part of the world', cannotBuild: "Distinction — participation is table stakes, not differentiation", risk: "Participating in everything means standing for nothing. The algorithm finds her but the audience can't describe her." },
            { force: 'Collaboration', requires: 'Working with other creators — shared content, joint launches, mutual amplification', builds: 'Audience crossover, industry legitimacy, the social proof of being chosen by peers', cannotBuild: "Trust — audiences know collabs are strategic. The relationship feels professional even when it's real.", risk: "Being known primarily as someone else's collaborator. The attribution problem." },
            { force: 'Viral Moments', requires: 'One post that escapes its origin cluster and reaches the platform-wide feed', builds: "Sudden follower growth, brand attention, the before-and-after of her public identity", cannotBuild: "The audience that was there before — they often leave when the platform arrives", risk: "Growth faster than identity. Who is she to 300,000 new followers who don't know her history?" }
          ]
        }),
        category: 'franchise_law',
        severity: 'critical',
      ...
      /* ─── 08 · Social Reputation Damage & Recovery ─── */
      {
        title: 'Social Reputation Damage & Recovery',
        content: JSON.stringify({
          summary: "Certain events harm reputation. The damage is not the event — it's the gap between the event and the reputation the character had built. The bigger the reputation, the more specific the damage.",
          events: [
        ...
        severity: 'critical',
        always_inject: true,
        applies_to: JSON.stringify(['story_generation', 'reputation_arcs', 'drama_scenes']),
        source_document: 'social-personality-v1.0',
        source_version: '1.0',
        created_at: now,
        updated_at: now
      ...
      /* ─── 09 · Character Story Arcs ─── */
      {
        title: 'Character Story Arcs',
        content: JSON.stringify({
          summary: "Characters evolve over time. These arc types describe the external narrative shape of a character's journey in LalaVerse. They run parallel to — and in tension with — the internal arc stages tracked in the Character Registry.",
          arcs: [
        ...
        severity: 'critical',
        always_inject: true,
        applies_to: JSON.stringify(['story_generation', 'character_arcs', 'narrative_structure']),
        source_document: 'social-personality-v1.0',
        source_version: '1.0',
        created_at: now,
        updated_at: now
      ...
      /* ─── 10 · Personality × Algorithm Interaction ─── */
      {
        title: 'Personality × Algorithm Interaction',
        content: JSON.stringify({
          summary: "The Timeline Engine and the Personality Engine interact. A character's trait profile determines how the algorithm responds to their behavior — and how their behavior responds to the algorithm over time.",
          combinations: [
            ...
            { traits: 'Low attention-seeking + high ambition', pattern: 'Builds behind the scenes — brands, products, influence through others rather than direct audience performance', algorithm: "Low personal content reach — the algorithm doesn't reward what it can't see", longTerm: "She builds something real while everyone else is building an audience. The question of which matters more." },
        ...
        severity: 'critical',
        always_inject: true,
        applies_to: JSON.stringify(['story_generation', 'character_profiles', 'algorithm_behavior', 'feed_scenes']),
        source_document: 'social-personality-v1.0',
        source_version: '1.0',
        created_at: now,
        updated_at: now
      ...
      /* ─── 11 · JustAWoman's Profile Note ─── */
      {
        title: "JustAWoman's Personality Profile",
        content: JSON.stringify({
          summary: "JustAWoman's baseline personality scores and narrative positioning.",
          profile: {
            attentionSeeking: 75,
            authenticity: 85,
            ambition: 95,
            dramaSensitivity: 40,
            trendResponsiveness: 60,
            communityOrientation: 80
          },
          notes: [
        ...
        severity: 'critical',
        always_inject: true,
        applies_to: JSON.stringify(['story_generation', 'character_profiles', 'justawoman']),
        source_document: 'social-personality-v1.0',
        source_version: '1.0',
        created_at: now,
        updated_at: now
