...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    try {
      ...
      await queryInterface.addColumn('assets', 's3_key_raw', {
        type: Sequelize.STRING(500),
        allowNull: true,
      }, { transaction });

      await queryInterface.addColumn('assets', 's3_url_raw', {
        type: Sequelize.STRING(500),
        allowNull: true,
      }, { transaction });

      await queryInterface.addColumn('assets', 'file_size_bytes', {
      ...
      await queryInterface.addColumn('assets', 's3_key_processed', {
        type: Sequelize.STRING(500),
        allowNull: true,
      }, { transaction });

      await queryInterface.addColumn('assets', 's3_url_processed', {
        type: Sequelize.STRING(500),
        allowNull: true,
      }, { transaction });

      await queryInterface.addColumn('assets', 'processed_file_size_bytes', {
        type: Sequelize.INTEGER,
        allowNull: true,
      }, { transaction });
      ...
      await queryInterface.addColumn('assets', 'width', {
        type: Sequelize.INTEGER,
        allowNull: true,
      }, { transaction });

      await queryInterface.addColumn('assets', 'height', {
        type: Sequelize.INTEGER,
        allowNull: true,
      }, { transaction });
      ...
      await queryInterface.addColumn('assets', 'processing_job_id', {
        type: Sequelize.STRING(255),
        allowNull: true,
      }, { transaction });

      await queryInterface.addColumn('assets', 'processing_error', {
        type: Sequelize.TEXT,
        allowNull: true,
      }, { transaction });

      await queryInterface.addColumn('assets', 'processed_at', {
        type: Sequelize.DATE,
        allowNull: true,
      }, { transaction });
  ...
  down: async (queryInterface, _Sequelize) => {
    ...
    try {
      ...
      // Drop ENUM type
      await queryInterface.sequelize.query('DROP TYPE IF EXISTS enum_assets_approval_status;', { transaction });
