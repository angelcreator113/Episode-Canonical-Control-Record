...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    try {
      ...
      // ──────────────────────────────────────────────
      const assetColumns = [
        { name: 'entity_type',           sql: 'VARCHAR(50)' },  // Use VARCHAR instead of ENUM for flexibility
        { name: 'character_name',        sql: 'VARCHAR(100)' },
        { name: 'outfit_name',           sql: 'VARCHAR(255)' },
        { name: 'outfit_era',            sql: 'VARCHAR(100)' },
        { name: 'transformation_stage',  sql: 'VARCHAR(50)' },  // Use VARCHAR instead of ENUM
      ...
      // ──────────────────────────────────────────────
      // 5. Create episode_wardrobe_defaults table (if not exists)
      // ──────────────────────────────────────────────
      const [wardrobeTableExists] = await queryInterface.sequelize.query(
        `SELECT EXISTS (
          SELECT FROM information_schema.tables 
          WHERE table_schema = 'public' 
          AND table_name = 'episode_wardrobe_defaults'
        ) as exists`,
        { transaction }
      );

      if (!wardrobeTableExists[0].exists) {
      ...
      // ──────────────────────────────────────────────
      // 6. Create asset_usage_log table (if not exists)
      // ──────────────────────────────────────────────
      const [usageLogTableExists] = await queryInterface.sequelize.query(
        `SELECT EXISTS (
          SELECT FROM information_schema.tables 
          WHERE table_schema = 'public' 
          AND table_name = 'asset_usage_log'
        ) as exists`,
        { transaction }
      );

      if (!usageLogTableExists[0].exists) {
  ...
  down: async (queryInterface, Sequelize) => {
    ...
    try {
      ...
      const assetCols = [
        'entity_type', 'character_name', 'outfit_name', 'outfit_era',
