...
(shouldSkip ? describe.skip : describe)('Episodes API Integration Tests', () => {
  ...
  describe('GET /api/v1/episodes', () => {
    it('should list episodes with pagination', async () => {
      const res = await authGet('/api/v1/episodes').query({ page: 1, limit: 10 });
    ...
    it('should reject invalid page parameter', async () => {
      const res = await authGet('/api/v1/episodes').query({ page: 'invalid', limit: 10 });
    ...
    it('should reject invalid limit parameter', async () => {
      const res = await authGet('/api/v1/episodes').query({ page: 1, limit: 200 });
    ...
    it('should reject page less than 1', async () => {
      const res = await authGet('/api/v1/episodes').query({ page: 0, limit: 10 });
    ...
    it('should filter by status', async () => {
      const res = await authGet('/api/v1/episodes').query({ status: 'approved', page: 1, limit: 10 });
    ...
    it('should reject invalid status filter', async () => {
      const res = await authGet('/api/v1/episodes').query({ status: 'invalid_status' });
    ...
    it('should search by title/description', async () => {
      const res = await authGet('/api/v1/episodes').query({ search: 'test', page: 1, limit: 10 });
    ...
    it('should reject search string that is too long', async () => {
      ...
      const res = await authGet('/api/v1/episodes').query({ search: longSearch });
    ...
    it('should handle multiple query parameters together', async () => {
      const res = await authGet('/api/v1/episodes').query({
        page: 2,
        limit: 20,
        status: 'approved',
        search: 'episode',
  ...
  describe('GET /api/v1/episodes/:id', () => {
    it('should reject request with invalid UUID format', async () => {
      const res = await authGet('/api/v1/episodes/invalid-id');
    ...
    it('should handle request with valid UUID format', async () => {
      ...
      const res = await authGet(`/api/v1/episodes/${validUUID}`);
    ...
    it('should return episode details if found', async () => {
      ...
      const listRes = await authGet('/api/v1/episodes').query({ limit: 1 });

      expect(listRes.status).toBe(200);

      if (listRes.body.data.length > 0) {
        const episodeId = listRes.body.data[0].id;
        const res = await authGet(`/api/v1/episodes/${episodeId}`);
  ...
  describe('End-to-End Episode Workflow', () => {
    it('should complete full episode viewing workflow', async () => {
      // 1. List episodes
      const listRes = await authGet('/api/v1/episodes').query({ page: 1, limit: 10 });
      ...
      if (listRes.body.data.length > 0) {
        ...
        const detailRes = await authGet(`/api/v1/episodes/${episode.id}`);
    ...
    it('should filter and browse episodes', async () => {
      ...
      const approvedRes = await authGet('/api/v1/episodes').query({ status: 'approved', page: 1, limit: 5 });

      expect(approvedRes.status).toBe(200);

      // Get pending episodes
      const pendingRes = await authGet('/api/v1/episodes').query({ status: 'pending', page: 1, limit: 5 });
  ...
  describe('Error Handling', () => {
    it('should return proper error for non-existent episode', async () => {
      ...
      const res = await authGet(`/api/v1/episodes/${nonExistentId}`);
    ...
    it('should handle database errors gracefully', async () => {
      ...
      const res = await authGet('/api/v1/episodes').query({ page: -1 });
