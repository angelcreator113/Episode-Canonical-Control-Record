/**
 * Canonical Asset Roles - Frontend
 * 
 * This is a copy of the backend canonical roles for use in React components.
 * 
 * CRITICAL: Keep in sync with backend src/constants/canonicalRoles.js
 * These roles define the contract between all thumbnail system components.
 */
...
export const getOptionalRoles = () => {
  return Object.entries(CANONICAL_ROLES)
    .filter(([_, config]) => !config.required)
    .map(([role]) => role);
};

export const getTextFieldRoles = () => {
  return Object.entries(CANONICAL_ROLES)
    .filter(([_, config]) => config.isTextField)
    .reduce((acc, [role, config]) => ({ ...acc, [role]: config }), {});
};

export const getAutoManagedRoles = () => {
  return Object.entries(CANONICAL_ROLES)
...
export default CANONICAL_ROLES;
