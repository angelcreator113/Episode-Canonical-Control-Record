...
/* ═══════════════════════════════════════════════════════════════
   ...
   10 tabs · Amber accent (#b89060) · .cm-* CSS prefix
   ═══════════════════════════════════════════════════════════════ */

const TABS = [
  { key: 'types',       label: 'Memory Types' },
  { key: 'strength',    label: 'Strength Levels' },
  { key: 'archives',    label: 'Archives' },
  { key: 'anniversary', label: 'Anniversaries' },
  { key: 'nostalgia',   label: 'Nostalgia Waves' },
  { key: 'legends',     label: 'Legends' },
  { key: 'feuds',       label: 'Historical Feuds' },
  { key: 'capsules',    label: 'Time Capsules' },
  { key: 'living',      label: 'Living History' },
  { key: 'rankings',    label: 'Influence Rankings' },
];

/* ─── DATA ─── */

const MEMORY_TYPES = [
  { type: 'Historic Fashion Moment', created: 'A designer debut or outfit appearance that changes what fashion is allowed to be in LalaVerse', example: 'A revolutionary silhouette at Velvet Season. An award show look that gets discussed at every Velvet Season that follows.', referenced: '"That look is giving Velvet Season \'22 revolution energy."', story: 'The designer who made it. The creator who wore it. The two people who argued about whether it was brilliant or derivative.', icon: '👗' },
...
const ARCHIVES = [
  { name: 'The Velvet Archive', maintained: 'Velvet City institutions + The Velvet Report', tracks: 'Runway collections, style revolutions, designer legacies, Atelier Circuit history', leaves_out: 'Street style, underground fashion, everything that didn\'t receive an invitation', control: 'The designers with the longest relationships with Velvet City institutions. What got archived is what they valued.', accent: '#d4789a' },
  { name: 'Glow Archives', maintained: 'The Glow Institute + Glow Gazette', tracks: 'Cosmetic innovations, viral beauty trends, technique origins, Glow Honors history', leaves_out: 'Salon-level invention — the techniques that started in Glow District that didn\'t get attributed', control: 'The beauty brands with the longest advertising relationships with the Glow Gazette.', accent: '#a889c8' },
  { name: 'The Creator Chronicle', maintained: 'Creator Harbor community + the Creator Conservatory', tracks: 'Influencer milestones, viral moments, creator economy breakthroughs, platform history', leaves_out: 'The creators who built significant audiences but never received institutional recognition', control: 'Whoever was prominent in Creator Harbor when the Chronicle was being written.', accent: '#7ab3d4' },
...
const CAPSULE_TYPES = [
  ...
  { type: 'The Year in Fashion / Beauty', made_by: 'Velvet Archive + Glow Archives annual compilation', included: 'Collections, looks, and techniques that defined the year\'s aesthetic', left_out: 'Underground aesthetics that influenced the mainstream without being credited', truth: 'The Year in Fashion is a story told by the people who were invited. The year had other stories.' },
];

const REFERENCE_TYPES = [
  { type: 'Era comparison', example: '"This aesthetic is pure Velvet Season \'22 energy."', function_: 'Establishes that LalaVerse has a documented past — the world is old enough to have eras.' },
  ...
  { type: 'Archive citation', example: '"The Velvet Archive has her first collection documented."', function_: 'The archive is legible to characters — they can access history.' },
...
const RANKING_METRICS = [
  ...
  { metric: 'Collaborations created', measures: 'Significant partnerships — joint launches, brand creations, mentor relationships', measured_by: 'Creator Chronicle + Velvet Archive cross-reference', misses: 'Private collaborations that shaped careers without a public record.' },
      ...
      <div className="cm-type-grid">
        <EditableList constantKey="MEMORY_TYPES" defaults={MEMORY_TYPES} label="Add Memory Type">
          ...
          <div className="cm-type-card">
            ...
            <div className="cm-type-field"><strong>What Creates It</strong><p>{m.created}</p></div>
            <div className="cm-type-field"><strong>Example</strong><p>{m.example}</p></div>
        ...
        <EditableList constantKey="STRENGTH_LEVELS" defaults={STRENGTH_LEVELS} label="Add Strength Level">
          ...
          <div className="cm-strength-card" style={{ borderLeftColor: s.color }}>
            <div className="cm-strength-header">
              ...
              <h4>{s.name}</h4>
              <span className="cm-strength-lifespan">{s.lifespan}</span>
            </div>
            <div className="cm-strength-field"><strong>How It's Referenced</strong><p>{s.referenced}</p></div>
...
function renderArchives() {
  return (
    <div className="cm-section">
      <p className="cm-intro">
        Who controls the archive controls the memory. These institutional archives are maintained by specific media networks and cultural institutions.
      </p>

      <div className="cm-power-callout">
        <span className="cm-power-icon">📜</span>
        <div>
          <strong>Memory Is Power</strong>
          <p>Who controls the archive controls the history. Who names the era controls what the era means. Who gets credited for the trend controls the legacy.</p>
        </div>
      </div>
        ...
        <EditableList constantKey="CAPSULE_TYPES" defaults={CAPSULE_TYPES} label="Add Capsule Type">
          ...
          <div className="cm-capsule-card">
            ...
            <div className="cm-capsule-columns">
              <div className="cm-capsule-col included"><strong>What Gets Included</strong><p>{c.included}</p></div>
              <div className="cm-capsule-col omitted"><strong>What Gets Left Out</strong><p>{c.left_out}</p></div>
            </div>
