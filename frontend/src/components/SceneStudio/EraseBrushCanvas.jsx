...
export default function EraseBrushCanvas({
  ...
  const promptSuggestions = useMemo(() => [
    'Seamless original background reconstruction',
    'Natural continuation with matching perspective',
    'Preserve lighting, texture, and depth',
    'Empty space matching surrounding environment',
    'Sky with consistent clouds and atmosphere',
    'Greenery and foliage matching nearby detail',
  ], []);
  ...
  const buildMaskDataUrl = useCallback(() => {
    ...
    const expansionPx = Math.max(2, Math.min(18, Math.round(brushSize * (brushMode === 'soft' ? 0.12 : 0.08))));
    const featherPx = Math.max(1, Math.min(10, Math.round(expansionPx * (brushMode === 'soft' ? 0.75 : 0.4))));
    ...
    expandedCtx.fillRect(0, 0, width, height);
    expandedCtx.globalCompositeOperation = 'lighter';

    const step = Math.max(1, Math.round(expansionPx / 3));
    for (let offsetY = -expansionPx; offsetY <= expansionPx; offsetY += step) {
      for (let offsetX = -expansionPx; offsetX <= expansionPx; offsetX += step) {
        if ((offsetX * offsetX) + (offsetY * offsetY) > expansionPx * expansionPx) continue;
        expandedCtx.drawImage(sourceCanvas, offsetX, offsetY);
      }
    }
    expandedCtx.drawImage(sourceCanvas, 0, 0);
    ...
    exportCtx.filter = `blur(${featherPx}px)`;
    exportCtx.drawImage(expandedCanvas, 0, 0);
  ...
  const handleApply = useCallback(() => {
    ...
    onApply(maskDataUrl, {
      prompt: customPrompt || 'Remove the selected object completely and reconstruct the background as if nothing had ever been there. Preserve the original perspective, depth, lighting, shadows, texture, and color balance. Extend surrounding structures and patterns seamlessly. Do not add new objects, blur, seams, artifacts, or repeated textures.',
      strength,
      variationCount,
    });
  }, [hasStrokes, isProcessing, onApply, customPrompt, strength, variationCount, buildMaskDataUrl]);
