...
The step 2 ratchet (model vs migrations) ends with `baseline scripts/schema-agreement-step2.baseline: N entries, X new, Y no longer occur`; paste it and any `NEW` / `GONE` lines. Keys are `Model<TAB>column<TAB>kind` (`missing-column`, `paranoid-no-deleted_at`, `no-table`). A `NEW` line (exit 1) is a model declaring a column, a paranoid `deleted_at` or a table that no live migration in `src/migrations/` creates: add the migration or fix the model's mapping, do not add it to the baseline. Migrations are not production: this gate says nothing about the live database (that is `scripts/check-pending-migrations.js` and `docs/MIGRATION_DRIFT_READ.md`). `GONE` lines do not fail; regenerate with `--step2-baseline scripts/schema-agreement-step2.baseline --update-baseline` in a PR that shrinks the set.
