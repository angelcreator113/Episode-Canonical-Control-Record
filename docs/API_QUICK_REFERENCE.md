...
> document was written January–March 2026. At this basis it documents **4**
> route groups (Episodes, Thumbnails, Metadata, Processing Queue) against
> **97** distinct `/api/v1` prefixes mounted in `src/app.js` — a live count
> derived by grep against that file at this basis, not carried from
> `PROJECT_CONTEXT.md`. For the current route list, read `src/app.js`
> directly; this document does not reflect it.
>
