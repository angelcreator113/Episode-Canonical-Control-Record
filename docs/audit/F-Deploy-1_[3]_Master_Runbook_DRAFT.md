...
>
> Sec 5 of this document was a permitted cold-entry read. That permission is revoked.
...
2. **[FD-31] Rotation - ALREADY EXECUTED 2026-06-12, do NOT repeat.** The canon `-dev`
  ...
  no-op; proceed to step 3 confirmation.
    ...
    **[RESOLVED 2026-06-16 - Stop Gate #1]** Count resolved against live AWS artifacts (SSM get-parameter-history + CloudTrail ModifyDBInstance, read 2026-06-16). Two canon rotation episodes are on record. Episode A (06-12): three masterUserPassword API calls on episode-control-dev (14:18 applyImmediately=false, 14:35 + 14:57 applyImmediately=true); whether one or multiple password values were set is indeterminate (CloudTrail redacts the value). Episode B (06-15): one masterUserPassword call at 09:50 paired to the SSM v2 write at 09:53. No-repeat-at-cutover is unchanged and remains mandatory. Historical control-gap: the SSM backup-of-record was first written 06-14 (v1), ~2 days after Episode A; recorded for history, NOT a Gate 2.5 reopen trigger. Authority for runtime verification stays SSM v2 + live auth probe (see Step 3).
...
8. **Post-cutover security sweep** (FD-31 §6.3 step 8): close `0.0.0.0/0` on the RDS SGs
  (**[CORRECTED 2026-06-16 - AF lineage restored to source.]** Per the AF birth record (`8043a591` / #722) and #808/#809, **F-Deploy-G1-AF is a class finding: all three RDS SGs admitted 5432 from `0.0.0.0/0` at filing, prod included. It is not a single SG.** Member states now differ and are tracked separately: (1) Canon SG `sg-002578912805d1930` (guards canon RDS 10.0.20.224; instance named `episode-control-dev` but holds live canon - naming inversion): `0.0.0.0/0` removed during 06-14 containment, narrowed to four explicit CIDRs. Was-it-used INDETERMINATE - no VPC flow logs on canon VPC `vpc-0754967be21268e7e`; containment closed the hole but did not establish whether anything traversed it while open. Carry as investigation item, not closed. (2) Fork SG `sg-0164d0b20fbebacbb` (empty `episode-control-prod`): still open `0.0.0.0/0` on :5432. Separate post-[3] sweep close item. (3) Staging SG: unverified - confirm at fold-in. Standing escalation trigger: `0.0.0.0/0` or `3.94.166.174/32` reappearing on canon ingress :5432 (both removed 06-14). AF authority is `8043a591` + `G1_Audit.md`, NOT FD-40 Sec 4. **[CORRECTED-WRONG 2026-06-16 - retained verbatim for audit traceability. The block below asserted AF as a single canon SG with the fork SG relabeled separate-from-AF. This is the FD-40 Sec 4 single-SG relabel, retracted as wrong via #809; AF is class-level per 8043a591/#722. Preserved at-filing per additive-supersede; do NOT act on it. Grep note: this block carries live SG IDs under a wrong claim.]** _(at-filing, 2026-06-15:)_ **[CORRECTED 2026-06-15 — FD-40]** AF label inversion fixed; canon SG already contained. AF = the canon RDS SG `sg-002578912805d1930` (byte-exact — gate record Sec 4, v20). `sg-0164d0b20fbebacbb` is the empty fork SG, NOT canon and not the AF target; it remains an explicit post-[3] sweep close item as its own real exposure, separate from AF. State note (live): canon SG `sg-002578912805d1930` already had `0.0.0.0/0` removed during 06-14 containment (narrowed to four explicit CIDRs) — so the AF post-[3] item is residual hardening, not an open `0.0.0.0/0`. The fork SG `sg-0164d0b20fbebacbb` is still `0.0.0.0/0` on :5432 (gate record Sec 8) — a separate deferred item. Standing escalation trigger: `0.0.0.0/0` or `3.94.166.174/32` reappearing on canon ingress :5432. Confirm dev/staging SGs too) and on the prod box SG sg-05c3a6ed6eee7b3a6 (F-Deploy-G1-AE);
