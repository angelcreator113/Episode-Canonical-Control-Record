...
- §6.5.1 route-table audit of retained 2 NAT Gateways — post-soak work, separate from G2. v1.2 update: per §4.1 discovery, the retained NAT Gateways are in a different VPC than `episode-backend` (see §9.6); the post-G2 audit scope of this item is reframed accordingly.
...
**Current status:** G2 implementation plan at v1.2. v1.0 published 2026-05-26 as the contract for G2 execution; v1.1 published 2026-05-27 with three doc-hygiene review fixes; v1.2 published 2026-05-28 with §4.1 topology corrections and three filed sub-findings. G2 work has not yet started at v1.2 ship time. The executor (Evoni) ships against this contract at v1.2.
...
- **Availability zone:** us-east-1d (prod-AZ parity per Planning doc §12; matches `episode-backend`)
...
**Subnet-sharing rationale (v1.2):** The default VPC contains exactly one subnet per AZ. `episode-backend` occupies the only us-east-1d subnet. The contract's us-east-1d directive (Planning doc §12 + FD-26) combined with default-VPC placement therefore necessitates subnet-sharing with `episode-backend`. This is operationally equivalent to placing the two instances in adjacent dedicated subnets, since they remain isolated at the ENI/IP/SG level. The only cost is that a subnet-wide misconfiguration would affect both instances; given the subnet's configuration (public IP on launch, IGW egress) is stable and matches what dev needs, the shared-subnet posture is the simplest correct placement. The Planning doc §6.5.3 NAT-cost-zero framing that v1.0/v1.1 cited as a subnet-selection input is mooted by the topology fact recorded in §9.6 — there are no NAT-routed subnets in this VPC.
...
3. Document G2 close in a Fix Plan revision (likely v1.6) or as a standalone G2-close note in the audit directory.
...
4. If t3.small passes: update FD-27 in a Fix Plan v1.6 revision (instance class changed from t3.micro to t3.small per memory profile fail; cite §6.1 of this doc as evidence). Path A: this is a substantive plan change requiring version bump.
...
2. §4.2 memory profile check passed at locked instance class (t3.micro per FD-27, or t3.small if §6.1 fallback fired, which would require Fix Plan v1.6 documentation).
...
8. G2 close documented in a Fix Plan revision (v1.6 or named G2-close note) on main. v1.6 also carries the §4.2 memory profile observations and the FD-27 confirm-or-revise.
...
|---|---|---|
| §4.2 memory profile data (peak, baseline, workload footprint) folded into FD-27 audit trail | Data doesn't exist until §4.2 fires | At G2 close, add to Fix Plan v1.6 or G2-close note |
...
| Planning doc §6.5.1 route-table audit (retained 2 NAT Gateways) — scope reframed per §9.6 | Topology fact moved the destination | Post-G2, separate work item, scope reframed per §9.6 |
...
| Topology framing PE (full reconciliation of audit's "shared single EC2" elision vs. actual 5-VPC topology) | Surfaced more fully by v1.2 §4.1 discovery | When convenient post-G2; could land alongside Fix Plan v1.6 |
...
1. **G2 doc PR opened** on branch `claude/f-deploy-1-phase-b-g2-implementation` (v1.0), `claude/f-deploy-1-phase-b-g2-implementation-v1-1` (v1.1), or `claude/f-deploy-1-phase-b-g2-implementation-v1-2` (v1.2 revision), doc-only change. v1.0/v1.1/v1.2 all use `[skip-automerge]` token consistent with Fix Plan v1.4/v1.5.
...
*End of F-Deploy-1 Phase B G2 Implementation v1.2.*
*Author: Claude, with JustAWomanInHerPrime (JAWIHP) / Evoni.*
*Branch: `claude/f-deploy-1-phase-b-g2-implementation-v1-2` (to be created at file-write time).*
*Date: 2026-05-28.*
*Parent: Fix Plan v1.4 (PR #717, merged 2026-05-26); Fix Plan v1.5 (PR #721, merged 2026-05-28).*
*Companion: Phase B G1 Planning doc (PR #716, merged 2026-05-26).*
*v1.0: PR #718 (merged 2026-05-26).*
*v1.1: PR #719 (merged 2026-05-28).*
*Successor: TBD. Fix Plan v1.6 at G2 close to record the close in the decision register, fold in §4.2 memory profile observations, FD-27 confirm-or-revise, and possibly an F-Deploy-G1-AD/-AE/-AF batch update if their resolution has begun by then.*
