...
**Current status:** G2 implementation plan at v1.4. v1.0 published 2026-05-26 as the contract for G2 execution; v1.1 published 2026-05-27 with three doc-hygiene review fixes; v1.2 published 2026-05-28 with §4.1 topology corrections and three filed sub-findings; v1.3 published 2026-07-09 as the FD-48 citation-repair pass (pointers only, no operational change); v1.4 published 2026-07-14 as the class-commit bump v1.3 §3 pre-wrote — §4.2 fired FAIL at t3.micro (Fix Plan v1.41, kernel-attested OOM) and the §6.1 fallback executed and PASSED at t3.small (Fix Plan v1.42), so §4.1 now commits t3.small per §6.1 step 4. G2 execution is IN PROGRESS at v1.4: §4.1 substrate work and the §4.2 gate are complete; §4.3 is the next executable step. The executor (Evoni) ships against this contract at v1.4.
...
| 1.4 | 2026-07-14 | Evoni | Class-commit bump per this doc's own §3 forward-provision and §6.1 step 4. §4.2 executed: FAIL at t3.micro (Fix Plan v1.41 — kernel-attested OOM, driver killed at ≈474 MiB anon on a 913 MiB box); §6.1 fallback executed (Fix Plan v1.42 — stop/modify/start, W1 rule revoked pre-stop and replacement ratified); §4.2 re-run PASSED at t3.small (peak 1,121 MiB vs 1,679 bar, dual-channel). §4.1 instance class: t3.micro → t3.small. FD-27 REVISED in Fix Plan v1.42. §4.2 marked executed-closed; §4.3 FD-57 supersession pointer added. No other operational change. |
...
- **Instance class:** t3.small (per FD-27 as REVISED in Fix Plan v1.42 — the §4.2 fail did occur at t3.micro and §6.1's fallback executed and passed; see v1.4 history row)
...
> **v1.4 status: EXECUTED — CLOSED.** Fired FAIL at t3.micro 2026-07-14 (Fix Plan v1.41, kernel-attested); §6.1 fallback executed; re-run PASSED at t3.small same day (Fix Plan v1.42: baseline 374 MiB, peak 1,121 vs 1,679 bar, zero OOM, zero restarts). The procedure below is retained as the executed contract; §5.2 numbers of record live in Fix Plan v1.42 §3.
...
> **v1.4 pointer:** the mechanics below predate the deploy-dev.yml rewrite (FD-57, Fix Plan v1.30 §5, merged 2026-07-10): transport is now SSM RunCommand by instance tag — no SSH, no host secret; DEV_EC2_HOST is retired. The §4.3 *gate meaning* stands (first dev deploy lands on the new instance and serves); the step reduces operationally to the re-enablement ruling (Rule 7, `disabled_manually` since 2026-07-01) + first workflow_dispatch. Operational rewrite of this section is deferred to the §4.3 session's own revision if needed.
...
*End of F-Deploy-1 Phase B G2 Implementation v1.4.*
*Author: Claude, with JustAWomanInHerPrime (JAWIHP) / Evoni.*
*Branch: `docs/g2-implementation-v1-4`.*
*Date: 2026-07-14.*
