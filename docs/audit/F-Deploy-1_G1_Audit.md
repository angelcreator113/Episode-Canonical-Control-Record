...
## §8 Findings Registry (consolidated)

**Added 2026-06-02 (registry reconcile, v13 Sec 8.1).** The original audit (2026-05-15/16) filed findings **A through AA** inline in §3 and classified them by sub-form in §4; it never carried a single consolidated registry. This section adds one, and folds in the post-audit finding set **AB through AK** surfaced across Fix Plan v1.5 (#721), the G2 v1.2 contract (#722), the prod split-brain hazard, the 2026-05-30 auto-deploy incident, the AJ monitoring work, and the AK prod-deploy-workflow hazard (#747).

**Letter accounting:** 37 letters assigned (A–AK). **AH is VOID** — a sequencing letter-slip, no finding ever assigned, retained-not-reused (not recycled). **36 substantive findings** (27 original A–AA + 9 reconciled: AB, AC, AD, AE, AF, AG, AI, AJ, AK).

**Register-namespace note.** The G1 letter registry (this section) is distinct from the F-Deploy-1 *decision* register (FD-1…FD-37) and from the G2 register (`S4.2-*`). Where the same condition appears in two registers, both are kept and cross-referred — notably **AG** (G1) and **F-Deploy-G2-S4.2-A** (G2) are the same prod split-brain in two registers; AG is the short-form cross-ref to the canonical S4.2-A record. The G2 register also carries **S4.2-B** (Secrets-Manager assumption dead) and **S4.2-C** (leaked secrets; `ANTHROPIC_API_KEY` rotated 05-30, dev `postgres` password rotation deferred to cutover); those are **G2-register, not part of this G1 fold** — noted here only so they are not lost.

### §8.1 — Status table (reconciled set AB–AK)

Findings **A–AA** remain documented inline in §3 and classified in §4; not re-listed here.

| Letter | Finding (short) | Severity | Disposition |
|---|---|---|---|
| AB | Actions context `${{ }}` evaluated inside `run:` shell | correctness/injection | CLOSED — fixed #720, guarded v1.5 §8 (#721) |
| AC | "skip" path used exit-78 vs explicit skip-output + exit 0 | correctness | CLOSED — fixed #720 (#721) |
| AD | No IAM instance profile on `episode-backend` (`IamInstanceProfile: null`); static AWS keys on box | P1 | OPEN — post-[3] security sweep |
| AE | `episode-backend-sg` admits `0.0.0.0/0` on 22 / 3000 / 3002 / 80 / 443 | P1 | OPEN — post-[3] security sweep |
| AF | Prod RDS reachable from public internet on 5432 | P0 | OPEN — top of post-[3] security sweep |
| AG | Prod split-brain + schema fork | P0 | OPEN — freeze active; resolved in [3] schema-fork reconciliation |
| AH | **VOID** — letter-slip | — | retained-not-reused |
| AI | Automated dev-deploy reloaded the frozen prod box (2026-05-30) | P0 | CONTAINED (workflows disabled); residual cleanup → [3] |
| AJ | Prod had no availability monitoring; multi-day ELB 502 unalerted | P1 | REMEDIATED (monitoring live, v13 §3.13); canary + target-health alarm deferred |
| AK | "Deploy to Production" workflow drifted; manual dispatch performs unsafe writes/restarts | P1 | HARD BLOCKER on [3]; path (a) taken 2026-06-02 (workflow disabled — live containment); AK-3/AK-4 corrected in OPEN PR #752 (not on main); AK-1/AK-2 → cutover; AK-5 open |

### §8.2 — Folded finding entries (AB–AK)

**Finding F-Deploy-G1-AB (FD-29) — GitHub Actions context expressions evaluated inside `run:` shell blocks.** Workflow `run:` steps referenced `${{ ... }}` GitHub Actions context directly in shell, rather than binding the value into an `env:` block first — a correctness/injection surface. Remediation: pass context via `env:` and reference the shell variable. Source: fixed in #720 (`5067b976`); guard added in v1.5 §8 (#721, `0a9d3ecf`) as an `env:` audit across `validate` / `deploy-dev` / `deploy-production`. Disposition: CLOSED (fixed + guarded). Decision: FD-29.

**Finding F-Deploy-G1-AC (FD-30) — Workflow "skip" path used exit-78 instead of an explicit skip-output + exit 0.** A step signalled skip via exit code 78, brittle across runner/Actions behavior; replaced with an explicit skip-output flag and `exit 0`. Source: fixed in #720 (`5067b976`); v1.5 (#721, `0a9d3ecf`). Disposition: CLOSED. Decision: FD-30.

**Finding F-Deploy-G1-AD (P1) — No IAM instance profile on `episode-backend`; box authenticates with static AWS keys.** The prod EC2 instance carries no IAM role/instance-profile (`IamInstanceProfile: null`); AWS access is via long-lived static keys on the box, with the secrets-access mechanism opaque from AWS-side discovery. Should move to an instance profile (scoped role) and retire static keys. Source: filed in G2 v1.2 contract (#722, `8043a591`), which sequences resolution into the post-G2 sweep below AF. Disposition: OPEN — post-[3] security sweep (folds into [3] Sec 4 "move off static AWS keys to instance profile"). Severity P1: #722 assigns no explicit P-level but ranks it below AF; rated P1 here as a real exposure not actively exploited.

**Finding F-Deploy-G1-AE (P1) — `episode-backend-sg` admits `0.0.0.0/0`.** The prod box's security group is open to the world (ports 22 / 3000 / 3002 / 80 / 443) rather than scoped to the ALB / required sources. Source: #722 (`8043a591`), which records it as "this way for months; not actively burning but real exposure." Disposition: OPEN — post-[3] security sweep. Severity P1: #722 assigns no explicit P-level and characterizes it as real-but-not-actively-burning, sweep-priority below AF; rated P1 here on that basis.

**Finding F-Deploy-G1-AF (P0 — confirmed live) — Prod RDS reachable from the public internet on 5432.** The `episode-control-prod` RDS instance uses SG `sg-0164d0b20fbebacbb`, whose 5432 ingress includes **`0.0.0.0/0`** (plus `10.2.0.0/16` — stale deleted-VPC route, dead allow-list; `172.31.26.1/32`; `172.31.0.0/16`; `108.216.160.136/32` — likely maintainer home IP, confirm not stale). The prod DB port is reachable from the entire internet, gated only by password auth. This is also the sole reason the 2026-06-02 read-only re-verify reached canon RDS with no network change. #722 ranks AF the most serious of the three (AD/AE/AF), highest priority in the post-G2 sweep. Memory note flags all three RDS SGs as similarly open (incl. canon `-dev`); confirm the dev/staging SGs at fold-in — the prod one is verified. Source: #722 (`8043a591`); confirmed live in `F-Deploy-1_PROD_SplitBrain_HAZARD.md` Sec 2.4 / Sec 4. Disposition: OPEN — top of the post-[3] security sweep ("close `0.0.0.0/0` on RDS SGs").

**Finding F-Deploy-G1-AG (P0) — Prod split-brain + schema fork.** The prod box's running process serves `episode-control-dev`/`episode_metadata` (143 tables, populated — the live data), while the on-disk `.env` points at `episode-control-prod`/`episode_metadata` (171 tables, verified EMPTY). Three-axis divergence — host, credentials (per-instance distinct `postgres` passwords), AND schema lineage: the two DBs have **forked, not drifted** — 37 prod-only tables (a video/editing/script-tooling feature schema the live DB never received), 9 dev-only (incl. dual migration-framework bookkeeping `pgmigrations` + `SequelizeMeta`, and a `decision_log`/`decision_logs` singular/plural collision); **neither DB is a superset.** Any process reload silently cuts prod onto the empty DB — boots clean, serves nothing, throws no error. Related: FD-31; FD-36 (canon-only `.env` defuses at the box); AK-1/AK-2 (deploy workflow re-arms it); AI (the 05-30 automated reload that landed on this freeze). **Canonical detailed record: F-Deploy-G2-S4.2-A** (`F-Deploy-1_PROD_SplitBrain_HAZARD.md` Sec 4); this G1-registry entry is the short-form cross-ref. Disposition: OPEN, P0 freeze active — resolved only in [3] / the gated schema-fork reconciliation, with a verified `-dev` backup taken first. Reconciliation is schema-fork resolution (decide canon, settle the 37 prod-only tables, the log-table collision, the dual migration history), NOT a repoint.

**Finding F-Deploy-G1-AH — VOID (letter-slip).** AH was a sequencing slip — no finding was ever assigned to the letter; AI took the next free slot. Recorded VOID per v13 Sec 8.1, retained-not-reused (the letter is not recycled). Source: incident-doc sequencing note ("AI is next free after AH-the-slip").

**Finding F-Deploy-G1-AI (P0) — Automated dev-deploy reaches the frozen prod box via shared-compute wiring; an untagged `claude/**` PR reloaded prod live on 2026-05-30.** The static shared-compute coupling documented in G1-G has a confirmed *active* exploit path. An untagged `claude/**` PR (#728) triggered `Auto-merge to Dev` → push to `dev` → `Deploy to Development`, which SSHed to the prod box `episode-backend` (`54.163.229.144`, F-Deploy-G1-G) and ran `pm2 delete all` + `pm2 start --env development` + `pm2 save` + a migration step — reloading a process that was under a P0 split-brain freeze (AG / G2-S4.2-A). **No data lost, by luck only:** the deploy's `DEV_DB_HOST` secret happened to equal `episode-control-dev` (the live data), not the on-disk `.env`'s empty `-prod`; had it pointed at `-prod`, this was the silent-empty-DB catastrophe. Residual box state (folds into [3] reconciliation — do NOT "fix" piecemeal, any fix is another reload): new PIDs; `pm2 save` overwrote `dump.pm2` (same dump-poisoning mechanism as FD-35); app left on port 3002 (dev port), not 3000; Template Studio route-loading bug with `/episodes` + `/shows` not serving; on-disk `.env` still points at empty `-prod`, so the split-brain persists and a plain `pm2 restart` would still swap onto empty. Containment: `Deploy to Development` and `Auto-merge to Dev` both DISABLED 2026-05-30 — the `[skip-automerge]` commit tag is NOT a sufficient guard; the disabled workflows are. **Letter: AI confirmed** (next free after AH-the-slip). Source: `F-Deploy-1_INCIDENT_2026-05-30_prod-autodeploy.md`; canonical record `F-Deploy-1_PROD_SplitBrain_HAZARD.md` Sec 7; feeds Fix Plan v1.6. Disposition: CONTAINED (workflows disabled); residual cleanup folds into [3].

**Finding F-Deploy-G1-AJ (P1) — Prod had no availability monitoring; a multi-day ELB 502 went unalerted.** The prod ALB carried no CloudWatch alarm on 5xx and no alert path, so the 06-01-class outage (nothing healthy on 3000 → ELB 502) was discovered by hand, not paged. Remediation (now LIVE, v13 Sec 3.13): SNS topic `episode-metadata-availability-alerts` (confirmed subscription) + CloudWatch alarm `prod-availability-elb-5xx` on `HTTPCode_ELB_5XX_Count` ≥5/5min on `app/primepisodes-alb/75ba68945d7aa0bf`, alarm+OK to the topic. Known limitation: the 5xx-count alarm needs traffic to fire — a true zero-traffic dead window won't trip it; the synthetic-canary follow-on (deferred) closes that gap. Target-health alarm blocked until Track B corrects the TG port 3002→3000 (post-[3]). Source: #743 (plan) → implemented 06-01 (direct CloudWatch/SNS, no PR). Disposition: finding REMEDIATED (monitoring live); canary + target-health alarm deferred follow-ons. *(The remediation is live, but the finding still needed to land in the registry — this entry is that fold.)*

**Finding F-Deploy-G1-AK (P1) — "Deploy to Production" workflow drifted from live reality; manual dispatch performs unsafe writes/restarts.** Five-part cluster, one root cause: the prod-deploy path was written against the pre-06-01 4-app topology and never updated, so it now disagrees with both live state and main (#746). Sub-findings: **AK-1** `.env` `DB_HOST` unconditionally overwritten from `secrets.PROD_DB_HOST` (the DB block has no "only if set" guard; only optional API keys do) → re-arms FD-36 (`deploy-production.yml` ~:291, :319–334). **AK-2** migrations run against `secrets.PRODUCTION_DATABASE_URL` while runtime `.env` `DB_HOST` comes from `PROD_DB_HOST` — two distinct secrets = encoded split-brain; `migrate:up` failure swallowed via `|| echo "...warnings"` (`deploy-production.yml` ~:347; `deploy-production.sh` ~:204–208). **AK-3** PM2 stop/delete/start uses names that drifted from roles and from #746: stops/deletes `episode-api` (now DEV @3002), misses `episode-api-prod-hotfix` (actual PROD), `--only` targets `episode-api-dev`/`episode-worker-dev` that #746 removed, then `pm2 save --force` persists the wrong topology (same dump-poisoning mechanism as FD-35) (`deploy-production.sh` ~:217–238 — see citation correction below). **AK-4** nginx redeploy reintroduces the risk class Track B scoped out, with a prod-vhost-delete failure path (`sudo rm -f /etc/nginx/sites-enabled/episode-prod` on failed `nginx -t`) (`deploy-production.sh` ~:165–200). **AK-5 (sub)** five deploy artifacts on main, one wired (`.github/scripts/deploy-production.sh`), three orphaned under `scripts/deploy/` — classify, don't fix. Trigger: `workflow_dispatch` only (`deploy-production.yml:2-12`), phrase-gated ("DEPLOY TO PRODUCTION") — accident-prevention, not a safety boundary; cannot auto-fire. Source: #747 (`F-Deploy-G1-AK_ProdDeploy_Workflow_Hazard.md`, draft v0.1). **Letter: confirmed AK** (next free slot after AJ; AH void-but-retained).

> **AK disposition — 2026-06-02 update (reconciles the AK five-point gate-status addendum).**
> Disposition: HARD BLOCKER on [3] with a satisfiable five-point gate — EITHER `gh workflow disable "Deploy to Production"` for the window (**path (a) — TAKEN 2026-06-02**, verified), OR audit-correct all five (path (b), mandatory if the workflow is [3]'s restart vehicle). Path (b) is **partially advanced, NOT complete**:
> - **AK-3 — corrected in OPEN PR #752 (commit `64c62092`; NOT yet merged to main).** `deploy-production.sh` rewritten to the #746 topology; four regions fixed (stop/delete → prod-only; start → `--only episode-api-prod-hotfix` + shared-worker reload, dead `*-dev` block removed; `else` fallback no longer hardcodes a prod start name, now `exit 1`; health gate + both failure-path `pm2 logs` re-pointed to `episode-api-prod-hotfix`).
> - **AK-4 — corrected in OPEN PR #752 (commit `cdd52009`; NOT yet merged to main).** Destructive nginx failure path removed: on failed `nginx -t`, do NOT delete the vhost, do NOT reload, `exit 1` with a truthful message (failure `else`, ~:194-200). The in-place `sed` SSL-detection branch untouched.
> - **AK-1 / AK-2 — DEFERRED to [3] cutover.** `PROD_DB_HOST` and `PRODUCTION_DATABASE_URL` are write-only GitHub secrets (unverifiable via API); "correcting" them means a blind overwrite of a prod DB secret. Both are set deliberately as a cutover step (establishing canon), with the post-restart `/health` abort-check as the safety net. AK-2's swallowed `migrate:up` failure is addressed in the same pass.
> - **AK-5 — OPEN scoping note, parallel-safe, NOT [3]-blocked.** Classify the three orphaned `scripts/deploy/` artifacts; do not bundle under "resolved at [3]" — track as its own cleanup item.
>
> **Therefore the AK gate is NOT satisfied by #752** (2 of 5) — and #752 is itself **OPEN**, so even AK-3/AK-4 are not yet on main. The **live containment is path (a): `Deploy to Production` disabled**. At [3] the gate is cleared via cutover-completion of AK-1/AK-2 (+ AK-5 disposition) OR by path (a) remaining in force. Do NOT arrive at [3] assuming AK is closed because #752 exists — it is unmerged.
>
> **Citation correction (per gate-status addendum Sec 1).** The AK-3 citation `deploy-production.sh:~217-238` **under-counts** — the post-deploy health gate (`grep "episode-api.*online"`) and the two failure-path `pm2 logs episode-api` lines are the same drifted-names defect but sit outside that range; PR #752 fixed all of them. Read `:217-238` as the stop/start block only; the full corrected set is wider.

### §8.3 — Fold-in notes

- **Reconcile content-complete.** AB–AK all have entries; AI's gap is closed from the incident + hazard docs; AH resolved as a letter-slip (not a content-void).
- **Severities.** AB/AC are correctness fixes (CLOSED). AD = P1, AE = P1 (real exposures, not actively exploited, scheduled in the post-[3] sweep below AF; #722 assigns no explicit P-levels, only a relative ranking). AF = P0 (public-internet prod DB, per hazard doc). AG / AI = P0 (hazard doc). AJ = P1. AK = P1 (#747).
- **Shared-database-coupling — no new letter.** `F-Deploy-1_PROD_SplitBrain_HAZARD.md` Sec 5 asks whether prod-reading-a-dev-named-DB needs its own G1 letter beyond G1-G's shared *compute*. Decision: no new letter — the class is covered across G1-G (structural compute-coupling analog), AG (live split-brain state), and AK-2 (workflow-encoded form); its fix path is the [3] schema-fork reconciliation, which AG already owns. No orphan mechanism/locus/remediation a new letter would hold; minting one would re-describe AG/G1-G under a fresh ID (cf. AH-the-slip).
- **Register-namespace.** Keep AG (G1) and `F-Deploy-G2-S4.2-A` (G2) both; AG = short-form cross-ref. `S4.2-B` / `S4.2-C` are G2-register, flagged here, not folded.
- **Live-state — verified 2026-06-02 against `origin/main`.** Main HEAD = `285a913a` (#750). Merged & on main: #743–#750; #720/#721/#722; #746 (`99b0d887`, Track B ecosystem); #747 (`6e4f6986`, AK finding). NOT merged — OPEN as of this reconcile: #751, #752 (AK-3/AK-4 corrections, `64c62092`/`cdd52009`), #753, #754. The AK corrections are drafted, not landed; the AK entry reflects this. When #752 merges, flip AK-3/AK-4 from "open PR" to "on main."
