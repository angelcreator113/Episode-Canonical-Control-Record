...
**Status:** Phase B G2 implementation plan. Authored 2026-05-26 immediately after Fix Plan v1.4 merge (PR #717, commit `75f2052a`). Specifies the implementation sequence, observables, rollback procedures, and close criteria for α deployment.
...
**Current status:** G2 implementation plan locked at v1.0. G2 work has not yet started. The document is published as the contract for G2 execution; the executor (Evoni) ships against this contract.
...
Future revisions are expected if execution surfaces a fact that retroactively requires plan changes (e.g. memory profile in §4.2 reveals t3.micro is insufficient → §4.1 must commit to t3.small, requiring a v1.1 revision). Path A discipline applies: in-place edits are acceptable for typo/clarity fixes; substantive plan changes require version bump and explicit author note.
...
| §4.5 | `*-dev` PM2 app-def retirement (FD-28) | Shared instance's PM2 process tree contains only prod apps |
...
5. Merge to main via the standard squash-merge path (no `--admin` needed unless mergeStateStatus is UNSTABLE per §9.5 considerations).
...
4. `pm2 save` — persist the updated process list to disk (PM2 startup script will replay only prod apps after this point).
5. Locate any committed PM2 ecosystem config in the repo (e.g. `ecosystem.config.js` if present) that defines `*-dev` apps. Open a separate PR to remove those entries. Same workflow as §4.3 (branch, edit, PR, pipeline, merge).

**Gate criterion:**

- `pm2 list` on shared instance shows only prod apps; zero `*-dev` apps remaining.
- `pm2 save` persisted (verified via `pm2 startup` dry-run showing only prod apps in restart manifest).
- Repo-side PM2 ecosystem config (if any) updated and merged.
...
**On fail:** route to §6.4 rollback-exhaustion paragraph. After §4.5 ships, the planned rollback path narrows significantly.
...
After §4.5 ships, `*-dev` PM2 apps on the shared instance are deleted from disk and from PM2's persisted state. There is no planned rollback path — re-creating `*-dev` apps on the shared instance is rebuild-from-scratch using Planning doc §10 as the rebuild guide and is incident-response territory, not planned rollback. The Path A implication: §4.5 should not ship until §4.4 burn-in is unambiguously clean and the executor has confidence the new instance is the durable home for dev workload. If there is any doubt at the §4.5 gate, extend §4.4 burn-in by another 7 days rather than ship §4.5 and discover a problem with no rollback.
...
- `episode-dev-backend` memory peak ≤ §4.2 measured baseline + 10% (allows for natural drift; large deviation = investigate)
...
5. `*-dev` PM2 app definitions retired from shared `episode-backend` instance per §4.5; PM2 process tree on shared instance contains only prod apps.
6. PM2 ecosystem config in repo (if any) updated to remove `*-dev` entries; updates merged to main.
...
1. **G2 doc PR opened** on branch `claude/f-deploy-1-phase-b-g2-implementation`, doc-only change. May or may not warrant `[skip-automerge]` token (this doc is similar in shape to Fix Plan v1.4, which used the token; consistency argues for using it).
...
**Rule 7 gates** apply per-step inside §4 work, not just at the doc ship. Each of §4.1, §4.2, §4.3, §4.4, §4.5, §4.6 is a Draft → Confirm → Execute boundary for the executor.

---

*End of F-Deploy-1 Phase B G2 Implementation v1.0 (draft).*
*Author: Claude, with JustAWomanInHerPrime (JAWIHP) / Evoni.*
*Branch: `claude/f-deploy-1-phase-b-g2-implementation` (to be created at file-write time).*
*Date: 2026-05-26.*
