...
|---|---|---|
| Anything on the production box (`episode-backend`, 54.163.229.144): process manager, reboot, `.env`, deploy | The split-brain in `F-Deploy-1_PROD_SplitBrain_HAZARD.md` (on-disk `.env` pointing at the empty instance) is attested resolved: the `.env` already named canon at the 2026-09-18 reconciliation (`F-Deploy-1_Fix_Plan_v1.52.md` §Finding 1), and the post-lift migration check named canon (`F-Deploy-1_Deploy_2026-09-24_2026-09-25.md` §8.1). A restart still loads whatever `.env` says, so a wrong `.env` would silently move prod off canon. | The freeze is lifted (`F-Deploy-1_Fix_Plan_v1.53.md`, Evoni's ruling, 2026-09-20). Agent sessions still never `ssh`, `scp`, run `pm2`, connect to RDS, edit a server `.env`, or enable/dispatch workflows — Evoni performs every production action herself. Before a deploy's restart, §7.1. |
